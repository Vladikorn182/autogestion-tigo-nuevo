...
def mensaje_pendientes_antiguos(detalle: pd.DataFrame, dias_antiguedad: int, fecha_hoy: date, agrupado: bool = False) -> str:
    ...
    if agrupado:
        for (eh, socio), grupo in detalle.groupby(["EH", "Socio"], dropna=False):
            ...
            for i, (_, r) in enumerate(grupo.iterrows(), start=1):
                lineas.append(f"{i}. {r.get('Código cliente','S/D')} | {r.get('Nodo','S/D')} | {int(r.get('Días', 0)) if pd.notna(r.get('Días', None)) else 'S/D'} días")
            lineas.append("")
    else:
        lineas.append("📋 *Código | Nodo | Fecha | Días*")
        for i, (_, r) in enumerate(detalle.iterrows(), start=1):
            fecha = r.get("Fecha", "")
            fecha_item = pd.to_datetime(fecha).strftime("%d/%m") if pd.notna(fecha) and fecha != "" else "S/F"
            dias = r.get("Días", "")
            dias_txt = str(int(dias)) if pd.notna(dias) and str(dias) != "" else "S/D"
            lineas.append(f"{i}. {r.get('Código cliente','S/D')} | {r.get('Nodo','S/D')} | {fecha_item} | {dias_txt} días")
    lineas.append("✅ Favor priorizar estos códigos por antigüedad y reportar avance.")
    return "\n".join(lineas).strip()


def mostrar_pendientes_inst() -> None:
    st.title("📋 Pendientes de Instalación con Pago")
    st.caption("Filtra casos antiguos de 3 días hacia atrás y genera mensaje global para WhatsApp.")
    ...
    if detalle.empty:
        ...
        st.dataframe(detalle_filtrado[["Código cliente", "Nodo", "Fecha", "Días", "EH", "Socio", "Tipo venta"]], use_container_width=True, hide_index=True)

        st.subheader("📊 Resumen por socio")
        st.dataframe(resumen, use_container_width=True, hide_index=True)

        st.subheader("📲 Mensaje WhatsApp global")
        formato = st.radio("Formato", ["Código + nodo", "Agrupado por socio"], horizontal=True, key="pend_formato")
        texto = mensaje_pendientes_antiguos(detalle_filtrado, int(dias_antiguedad), fecha_hoy, agrupado=(formato == "Agrupado por socio"))
        st.text_area("Copiar mensaje", texto, height=330, key="pend_msg")
        st.markdown(f"[📲 Enviar por WhatsApp]({whatsapp_link(texto)})")

        excel = excel_bytes({"Resumen": resumen, "Antiguos +3 dias": detalle_filtrado, "Diagnostico": pd.DataFrame([diagnostico]).astype(str)})
...
def mensaje_suspendidas_global(detalle: pd.DataFrame, fecha_hoy: date, dias_antiguedad: int) -> str:
    fecha_txt = fecha_hoy.strftime("%d/%m/%Y")
    lineas = [
        f"🚨 *SUSPENDIDAS ANTIGUAS +{dias_antiguedad} DÍAS*",
        f"📅 Corte: *{fecha_txt}*",
        f"📌 Total casos: *{len(detalle)}*",
        "",
    ]
    ...
    lineas.append("📋 *Detalle por socio:*")
    for (eh, socio), grupo in detalle.groupby(["EH", "Socio"], sort=False):
        lineas.extend(["", f"👤 *{socio}*", f"🆔 EH: *{eh or 'S/D'}* | 🚨 *{len(grupo)}* casos"])
        for i, (_, r) in enumerate(grupo.iterrows(), start=1):
            lineas.append(f"{i}. {r.get('Código cliente','S/D')} | {r.get('Nodo','S/D')} | {r.get('Días','S/D')} días | {r.get('Categoría','Revisar')}")
    lineas.extend(["", "✅ Favor revisar, contactar al cliente y reportar el avance o motivo de suspensión."])
    return "\n".join(lineas)


def mostrar_suspendidas() -> None:
    st.title("🚨 Suspendidas")
    st.caption("Carga el archivo diario de suspendidas. Filtra casos antiguos y clasifica la observación CRM.")
    ...
    texto = mensaje_suspendidas_global(detalle, fecha_hoy, int(dias_antiguedad))
    st.text_area("Copiar mensaje global", texto, height=360, key="susp_msg")
    st.markdown(f"[📲 Enviar por WhatsApp]({whatsapp_link(texto)})")

    excel = excel_bytes({"Resumen por socio": resumen, "Resumen categoria": resumen_categoria, "Detalle": detalle, "Diagnostico": pd.DataFrame([diagnostico]).astype(str)})
...
def mostrar_pendientes_pago() -> None:
    ...
    st.caption("Carga el archivo de pendientes de pago para filtrar por socio/nodo y generar mensaje WhatsApp.")
    ...
    st.dataframe(detalle_filtrado, use_container_width=True, hide_index=True)

    resumen = (
        detalle_filtrado.groupby(["EH", "Socio"], as_index=False)["Código cliente"]
        .count()
    ...
    lineas = [
        ...
        "",
        "📋 *Código | Nodo | Teléfono*",
    ]
    for i, (_, r) in enumerate(detalle_filtrado.iterrows(), start=1):
        tel = r.get("Teléfono 2") or r.get("Teléfono 1") or "S/D"
        lineas.append(f"{i}. {r['Código cliente']} | {r['Nodo'] or 'S/D'} | {tel}")
    lineas.append("")
    ...
    st.subheader("📲 Mensaje WhatsApp")
    st.text_area("Copiar mensaje", texto, height=330, key="pago_msg")
    st.markdown(f"[📲 Enviar por WhatsApp]({whatsapp_link(texto)})")
...
def mostrar_inicio() -> None:
    st.markdown(
        ...
        <div style="padding: 30px; border-radius: 18px; background: linear-gradient(135deg, #0033A0, #00AEEF); color: white;">
            ...
            <p style="font-size: 18px;">Seguimiento de ventas, WhatsApp, agenda técnica, pendientes, pagos, suspendidas y objetivos.</p>
...
def main() -> None:
    ...
    st.sidebar.title("📊 Autogestión Tigo")
    ...
    if modulo == "Inicio":
        ...
        mostrar_suspendidas()
    elif modulo == "Objetivos Configuración":
        mostrar_objetivos_configuracion()
